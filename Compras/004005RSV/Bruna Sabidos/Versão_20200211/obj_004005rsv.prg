...
*!*-----------------------------------------------------USERESERVA-----------------------------------------------------------------------------
							...
							IF !f_vazio(Cur_centro_custo.centro_custo) then
								...
								* -joao carlos 2018.11.13 - usereserva - atribuindo o mês relacionado(Na mensagem não é utilizado - deixei apenas para se houver alguma utilidade futura)
								...
								if((xValorEmpenhado + xGasto) > (xValorOrcado))
									...
									messagebox('Detectamos que o pedido '+ xPedido +' possui Múltiplas Entegas  e item no valor de ' + cast(xValorEmpenhado as char(10)) + ' esta estourando o orçamento do centro de custo ' + cast(ALLTRIM(xDesc_custo) as varchar(20))  + ' para o mês de ' + cast(xMesDesc as varchar(10)) + '.' + CHR(13)+ CHR(13) +;
					...
					*-joao carlos 2018.11.13 - usereserva - atribuindo o mês relacionado(Na mensagem não é utilizado - deixei apenas para se houver alguma utilidade futura)
					...
					if((xValorEmpenhado + xGasto) > (xValorOrcado))
					 messagebox('Detectamos que o pedido '+ xPedido +' no valor de ' + cast(xValorEmpenhado as char(10)) + ' esta estourando o orçamento do centro de custo ' + cast(ALLTRIM(xDesc_custo) as varchar(20))  + ' para o mês de ' + cast(xMesDesc as varchar(10)) + '.' + CHR(13) + CHR(13) +;
								'Recomendamos que você faça um exercício de remanejamento ou prorrogação dessa despesa junto ao time de Orçamento, caso seja possível.',64,'Estouro Orçamento')

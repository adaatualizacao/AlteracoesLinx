...
*!*						SET STEP ON
*!*						*- usereserva - joao carlos - 2019.04.08 - Não permite salvar sem informações Projeto e Recorrente
*!*						if (f_vazio(v_rsv_orcamento_pedido_categoria.id_categoria) OR f_vazio(v_rsv_orcamento_recorrente_pedido.id_categoria))
*!*							messagebox("Você deve preencher o campo Projeto/Categoria ou Recorrente. Estes campos não podem ser nulos.")
*!*							return .f.
*!*						endif
					...
					*-joao carlos 2018.11.13 - usereserva - atribuindo o mês relacionado(Na mensagem não é utilizado - deixei apenas para se houver alguma utilidade futura)
					...
					messagebox('Este pedido está estourando o centro de custo ' + cast(ALLTRIM(xDesc_custo) as varchar(20))  + ' no mês '+cast(xMesDesc as varchar(10))+'. Dê uma olhada no demonstrativo abaixo:'+chr(13) + chr(13)  +;
					'Orç: ' + cast(xValorOrcado as char(10)) + CHR(13) + CHR(13)+;
					'Realizado: ' + cast(xEmpenhado as char(10)) + CHR(13) + CHR(13)+;
					'Estouro acumulado: ' +  cast(xValorEstouro as char(10))  + CHR(13) + CHR(13)+;
					'Recomendamos que você faça um exercício de remanejamento ou prorrogação dessa despesa junto ao time de Orçamento, caso seja possível.'+CHR(13)+CHR(13))
